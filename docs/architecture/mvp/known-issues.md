...
| **PLUME-AUTH-001** | Closed | BFF session maps access-JWT claims onto `HttpContext.User` (roles/`sub`); Invite requires `Roles=admin` ([plume#8](https://github.com/Bardie-radio/plume/issues/8)) |
| **PLUME-CONTRACT-001** | Open | Home create hardcodes `playback_access` / `control_access` wire strings (defaults, form options, normalize) — drift vs Kithara enums will break create; fix by referencing shared Kithara contracts ([plume#9](https://github.com/Bardie-radio/plume/issues/9)) |
